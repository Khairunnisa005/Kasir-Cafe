...
if (!empty($_SESSION['admin'])) {
    ...
    if (!empty($_GET['pelanggan'])) {
        ...
        print_r($data);
        echo "</pre>";
        $sql = mysqli_query($conn, "UPDATE pelanggan SET PelangganID='$id', NamaPelanggan='$nama' , Alamat='$alamat' , NomorTelepon='$telp' WHERE PelangganID='$id'");
        if (!$sql) {
            echo 'gagal';
        }
        print_r($data);
        ...
        // $row = $config -> prepare($sql);
        // $row -> execute($data);
        echo '<script>window.location="../../index.php?page=pelanggan/edit&pelanggan='.$id.'&success-edit=edit-data"</script>';
    }
    ...
    if (!empty($_GET['jual'])) {
        $id = htmlentities($_POST['id']);
        $id_penjualan = htmlentities($_POST['id_penjualan']);
...
if (!empty($_GET['carii_pelanggan'])) {
    $carii = trim(strip_tags($_POST['keyword']));
    if ($carii == '') {
    } else {
        $sql = "SELECT PelangganID, NamaPelanggan, Alamat, NomorTelepon FROM pelanggan
        WHERE PelangganID LIKE '%$carii%' OR NamaPelanggan LIKE '%$carii%' OR Alamat LIKE '%$carii%'";
        $row = $config -> prepare($sql);
        $row -> execute();
        $hasil2= $row -> fetchAll();
        ?>
    ...
    <?php foreach ($hasil2 as $hasilpl) {?>
        <tr>
            <td><?php echo $hasilpl['PelangganID'];?></td>
            <td><?php echo $hasilpl['NamaPelanggan'];?></td>
            <td><?php echo $hasilpl['Alamat'];?></td>
            <td><?php echo $hasilpl['NomorTelepon'];?></td>
            <td>
            <a href="fungsi/tambah/tambah.php?juall=juall&PelangganID=<?php echo $hasilpl['PelangganID'];?>&id_kasir=<?php echo $_SESSION['admin']['id_member'];?>" 
                class="btn btn-success">
