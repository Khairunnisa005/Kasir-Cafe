...
class view
{
    ...
    public function pelanggan_cari($cari)
    {
        $sql = "select pelanggan.*, PelangganID
                from pelanggan where PelangganID like '%$cari%' or NamaPelanggan like '%$cari%' or PelangganID like '%$cari%'";
        $row = $this-> db -> prepare($sql);
        $row -> execute();
        $hasil = $row -> fetchAll();
        return $hasil;
    ...
    public function pelanggan_id()
    {
        ...
        $urut = substr($hasil['PelangganID'], 2, 3);
        $tambah = (int) $urut + 1;
        if (strlen($tambah) == 1) {
            $format = 'PG00'.$tambah.'';
        } elseif (strlen($tambah) == 2) {
            ...
            $ex = explode('PG', $hasil['PelangganID']);
            $no = (int) $ex[1] + 1;
            $format = 'PG'.$no.'';
        }
        return $format;
    ...
    public function jual()
    {
        ...
        $sql = "SELECT * FROM jual ORDER BY 'id' DESC";
        $row = $this->db->prepare($sql);
        $row->execute();
        $hasil = $row->fetchAll();
        return $hasil;
    ...
    {
        $sql = "SELECT * FROM detailjual WHERE id_jual=?";
        $row = $this->db->prepare($sql);
        $row->execute(array($id));
        $hasil = $row->fetchAll();
        return $hasil;
