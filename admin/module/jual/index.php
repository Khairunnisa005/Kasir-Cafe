...
<?php if (isset($_GET['success-pelanggan'])) { ?>
    <div class="alert alert-success-pelanggan">
...
<!-- <div class="row">
    <div class="col-sm-4">
        <div class="card card-primary mb-3">
            ...
            <div class="card-body">
                <input type="text" id="carii" class="form-control" name="carii"
                    placeholder="Masukan : ID / Nama Pelanggan  [ENTER]">
            </div>
        </div>
    </div>
                    ...
                    <table class="table table-stripped">
                        ...
                        if (!empty($_GET['nota'] == 'yes')) {
                            ...
                            if (!empty($bayar)) {
                                ...
                                if ($bayar >= $total) {
                                    ...
                                    // Retrieve the last inserted PelangganID to generate the next one
                                    $sql = 'SELECT PelangganID FROM jual ORDER BY id DESC LIMIT 1';
                                    $row = $config->prepare($sql);
                                    $row->execute();
                                    $hasil = $row->fetch();

                                    // Generate the new PelangganID
                                    if ($hasil) {
                                        $urut = (int) substr($hasil['PelangganID'], 5); // Extract the numeric part from PelangganID
                                        $tambah = $urut + 1;
                                    } else {
                                        $tambah = 1; // If no records found, start with 1
                                    }

                                    // Conditional logic to construct the new PelangganID
                                    if ($tambah < 10) {
                                        $id_pelanggan = 'PLGGN00' . $tambah;
                                    } elseif ($tambah < 100) {
                                        $id_pelanggan = 'PLGGN0' . $tambah;
                                    } else {
                                        $id_pelanggan = 'PLGGN' . $tambah;
                                    }

                                    $PelangganID = $id_pelanggan;

                                    //Untuk menyimpan ke dalam tabel jual
                                    $d = array($PelangganID, $id_member[0], $total_bayar, $tgl_input);
                                    $sql1 = "INSERT INTO jual (PelangganID, id_member, total, tanggal_input) VALUES (?,?,?,?)";
                                    ...
                                    for ($x = 0; $x < $jumlah_dipilih; $x++) {
                                        ...
                                        //mengambil id dari table jual
                                        $jual = "SELECT * FROM jual WHERE tanggal_input='$tgl_input'";
                                        $row_jual = $config->query($jual);
                                        $hasil_id = $row_jual->fetchAll();
                                        $id_jual = $hasil_id[0]['id'];

                                        //insert data ke table detailjual
                                        $d = array($id_jual, $id_barang[$x], $jumlah[$x], $total1[$x]);
                                        $sql = "INSERT INTO detailjual (id_jual,id_barang,jumlah,subtotal) VALUES(?,?,?,?)";
                                        $row = $config->prepare($sql);
                                        $row->execute($d);
                        ...
                        <form method="POST" action="index.php?page=jual&nota=yes#kasirnya">
                            <?php foreach ($hasil_penjualan as $isi) {; ?>
                                ...
                                <input type="hidden" name="PelangganID[]" value="<?php echo $isi['PelangganID']; ?>">
